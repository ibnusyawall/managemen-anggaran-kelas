<template>
  <div>
    <b-card class="mb-4" no-body>
      ...
      <UangKeluarList @editData="editData" />
    </b-card>
    <template v-if="show_edit">
      <UangKeluarEdit :id_data="id_data" @updateData="updateData" />
    </template>
...
<script>
import UangKeluarList from "./components/UangKeluarList.vue";
import UangKeluarAdd from "./components/UangKeluarAdd.vue";
import UangKeluarEdit from "./components/UangKeluarEdit.vue";
...
export default {
  ...
  methods: {
    ...
    updateData({ id, form }) {
      ...
      axios
        .put(`/api/v1/uang-keluar/${id}`, form)
        .then((result) => {
